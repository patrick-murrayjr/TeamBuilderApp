...
document.querySelector('#player-form').addEventListener('submit', e => {
  ...
  if (
    ...
    UI.showPlayerAlert('Player Name and Position are required', 'danger', 1800);
  } else {
    let button = document.querySelector('#button-add');
    if (button.classList.contains('btn-player-add')) {
      if (
        DataStorage.searchForMatchingPlayer(player, DataStorage.getPlayers()) <
        0
      ) {
        // Add Player to UI
...
document.querySelector('#player-list').addEventListener('click', e => {
  ...
  if (e.target.classList.contains('edit')) {
    ...
    UI.displaySelectedPlayerOnForm(player);
    UI.showPlayerAlert(
      'Make changes to your selected player, to save without changes just click Edit Player',
      'warning',
      6000
    );
  }
